    ...
    try {
        let query = `
            SELECT 
                ...
                a.description, a.created_at, a.updated_at, a.scheduled_at
            FROM applications a
        ...
        query += ' ORDER BY a.scheduled_at ASC NULLS LAST, a.id DESC';
...
router.post('/', async (req, res) => {
    const { carId, staffId, description, scheduledAt } = req.body;
    try {
        await db.none(`
            INSERT INTO applications (car_id, staff_id, description, created_at)
            VALUES ($1, $2, $3, NOW())
        `, [carId, staffId, description, scheduledAt || null]);
...
router.put('/:id', async (req, res) => {
    ...
    try {
        await db.none(`
            UPDATE applications 
            SET car_id = $1, staff_id = $2, description = $3, scheduled_at=$4
            WHERE id = $5
        `, [carId, staffId, description, scheduledAt || null, req.params.id]);
